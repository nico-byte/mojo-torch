...
struct Tensor(Movable, Copyable):
    ...
    fn __matmul__(self, rhs: Tensor) -> Tensor:
        ...
        if self.shape.__len__() == 2 and rhs.shape.__len__() == 2:
            ...
            if k != n:
                print("MatMul not possible -> self.cols: " + String(k) + " != rhs.rows: " + String(n))
                return Tensor(0)
                ...
                for col_block in range(0, p, nelts * 4):
                    for k_val in range(k):
                        ...
                        fn dot[nelts_inner: Int](col: Int):
                            var c_val = C.data.load[width=nelts_inner](row * p + col + col_block)
                            var a_val = self.data.load(row * k + k_val)
                            var b_val = rhs.data.load[width=nelts_inner](k_val * p + col + col_block)
                            C.data.store[width=nelts_inner](row * p + col + col_block, c_val + a_val * b_val)
                ...
                fn calc_row_3d(row: Int):
                    ...
                    for col_block in range(0, p, nelts * 4):
                        for k_val in range(k):
                            @parameter
                            fn dot[nelts_inner: Int](col: Int):
                                var c_val = C.data.load[width=nelts_inner](c_offset + row * p + col + col_block)
                                var a_val = self.data.load(self_offset + row * k + k_val)
                                var b_val = rhs.data.load[width=nelts_inner](rhs_offset + k_val * p + col + col_block)
                                C.data.store[width=nelts_inner](c_offset + row * p + col + col_block, c_val + a_val * b_val)
    ...
    fn __matmul_tiled__(self, rhs: Tensor) -> Tensor:
        ...
        if self.shape.__len__() == 2 and rhs.shape.__len__() == 2:
            ...
            fn calc_tile_2d(jo: Int, io: Int):
                var accumulators = stack_allocation[tile_i * tile_j, Scalar[type]]()
                ...
                for k_val in range(k):
                    ...
                    fn calc_tile_row_2d(i: Int):
                        ...
                        fn calc_tile_cols_2d[nelts_inner: Int](j: Int):
                            ...
                            var b_val = rhs.data.load[width=nelts_inner](k_val * p + jo + j)
                ...
                for i in range(tile_i):
                    for j in range(tile_j):
                        if io + i < m and jo + j < p:
                            C.data.store((io + i) * p + (jo + j), accumulators[i * tile_j + j])
            ...
            fn calc_batch(b: Int):
                ...
                fn calc_tile_3d(jo: Int, io: Int):
                    ...
                    for k_val in range(k):
                        ...
                        fn calc_tile_row_3d(i: Int):
                            ...
                            fn calc_tile_cols_3d[nelts_inner: Int](j: Int):
                                var idx = i * tile_j + j
                                var a_val = self.data.load(self_base + (io + i) * k + k_val)
                                var b_val = rhs.data.load[width=nelts_inner](rhs_base + k_val * p + jo + j)
                                var acc = accumulators[idx]
                                accumulators[idx] = acc + a_val * b_val[0]
                    ...
                    for i in range(tile_i):
                        for j in range(tile_j):
                            if io + i < m and jo + j < p:
                                C.data.store(c_base + (io + i) * p + (jo + j), accumulators[i * tile_j + j])
                ...
                parallelize[tile_parallel_rows_3d]((m + tile_i - 1) // tile_i, m)
        ...
        for i in range(m):
            for kk in range(k):
                ...
                fn dot[nelts_inner: Int](j: Int):
                    ...
                    C.data.store[width=nelts_inner](i * p + j, c_val + a_val * b_val)
    ...
    fn transpose(self, axis1: Int, axis2: Int) -> Tensor:
        ...
        """
        if axis1 < 0 or axis1 >= self.shape.__len__() or axis2 < 0 or axis2 >= self.shape.__len__():
