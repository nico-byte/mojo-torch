...
fn main():
    ...
    print("Time taken for matmul (vectorized + parallelized): ", (end - start)/1e6, " milliseconds")

    start = perf_counter_ns()
    var _ = a.__matmul_tiled__(b)  # Uses tiled accumulation
    end = perf_counter_ns()
    print("Time taken for matmul (tiled): ", (end - start)/1e6, " milliseconds")

    start = perf_counter_ns()
    var _ = a.__matmul_simd__(b)   # Uses pure SIMD
    end = perf_counter_ns()
    print("Time taken for matmul (SIMD): ", (end - start)/1e6, " milliseconds")
    ...
    print("Output shape from Linear layer: [", output.shape[0], ", ", output.shape[1], "]")
